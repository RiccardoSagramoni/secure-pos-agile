...
class CommunicationController:
    ...
    def send_testing_execution(self, session_id):
        ...
        response = requests.post("http://127.0.0.1:1234/", json=result_dict)
        if not response.ok:
            logging.error("Failed to send result:\n%s", result_dict)
    
    def send_testing_development(self):
        timestamp = datetime.now()
        timestamp_str = timestamp.strftime("%Y-%m-%d %H:%M:%S.%f")
        result_dict = {
            "scenario_id": 0,
